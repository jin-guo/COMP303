...
5. In case your JUnit setup is not ready, you can add JUnit from Project Settings -> Libraries -> "+" -> From Maven -> Enter "org.junit.jupiter:junit-jupiter-api:jar:6.0.1" (or the latest version) as query and locate the Junit.
6. If you have runtime error for JavaFX `javafx components are missing`, please add the VM option “--module-path **Your_JavaFX_lib_path** --add-modules=javafx.controls,javafx.fxml" in your configurations. If your JDK24 or a newer version, you will also need to add this VM option: *"--enable-native-access=javafx.graphics"*.<sup>[3](#footnote3)</sup>
...
<a name="footnote2">2</a>: 2X indicates the latest JDK version number. For example, the latest one in Jan 2026 is JDK 25.

<a name="footnote2">3</a>: This is to suppress the warning with the following message: `WARNING: Restricted methods will be blocked in a future release unless native access is enabled`. See [here](https://openjfx.io/highlights/24/) for more information.
